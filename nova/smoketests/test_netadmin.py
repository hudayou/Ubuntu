...
class SecurityGroupTests(base.UserSmokeTestCase):
    ...
    def test_004_can_access_metadata_over_public_ip(self):
        ...
        self.scp("socat", "root@%s:/usr/bin/socat" % (ip), TEST_KEY)
        conn = self.connect_ssh(ip, TEST_KEY)
        stdin, stdout, stderr = conn.exec_command('socat TCP-LISTEN:80,reuseaddr,fork TCP:169.254.169.254:80 &')
        conn.close()
        # Give instance time to start socat.
        time.sleep(5)
        self.assertTrue(self.__public_instance_is_accessible())
