...
class _WriteDecorator:
    ...
    def write(self, arg):
        self.stream.write('\n')
        self.stream.write(('        '+arg).ljust(64))


class SmokeTestCase(unittest.TestCase):
    def no_capture_write(self, output):
        stdout = sys.stdout
        sys.stdout = sys.__stdout__
        sys.stdout.write(output)
        sys.stdout = stdout

    def scp(self, src, dest, key_name):
        cmd = 'chmod 0600 /tmp/%s.pem && scp -o StrictHostKeyChecking=no -i /tmp/%s.pem %s %s && chmod 0644 /tmp/%s.pem' % (key_name, key_name, src, dest, key_name)
